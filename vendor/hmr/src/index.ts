...
class Hmr extends Service {
  ...
  constructor(ctx: Context, public config: Hmr.Config) {
    ...
    if (this.internal === undefined && config.root.length > 0) {
      throw new Error('--expose-internals is required for HMR module watching')
    }
    this.baseDir = fileURLToPath(new URL(config.base || '.', ctx.baseUrl))
  }

  /** Return the Node module loader required by module-watch operations. */
  ...
  private async _resolve(specifier: string, parentURL: string, attrs: ImportAttributes): Promise<ResolveResult> {
    const internal = this.moduleLoader()
  ...
  async* [Service.init]() {
    ...
    const onChange = (kind: 'add' | 'change' | 'unlink', path: string) => {
      ...
      if (loader.internal?.loadCache.has(url) === true) {
  ...
  async getLinked(url: string) {
    const job = this.moduleLoader().loadCache.get(url)
    if (!job) return []
