...
Configuration watching and module hot reload have different requirements. An explicit HMR instance with `root: []` watches `cordis.patch.yml` through the filesystem only and works without Node internals. Packaged embedders may also omit `process.argv[1]`; the config-only instance then treats the main-entry externals set as empty instead of resolving a fabricated script path. Non-empty module roots still require the internal loader and fail at startup when it is absent. Pass the profile directory as HMR `base`; otherwise a process-level `chdir()` can move the watched path.
...
| `vendor/hmr/src/index.ts` | Keep config-only `root: []` independent from module-loader internals, retain the guard for non-empty module roots, and leave main-entry externals empty when an embedder has no `process.argv[1]`. Reapply the local-modification log entry after sync. | Electron boot fails with `--expose-internals` or `path.resolve(undefined)`, or module HMR runs without required cache data. |
...
| Packaged boot reports `The "paths[0]" argument must be of type string` | The config-only HMR main-entry walk must tolerate an absent `process.argv[1]`. |
