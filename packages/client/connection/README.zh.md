...
`electron` Host transport 无需 `ctx.webServer` 即可注册相同的 `/api` Fetch handler 与逻辑 RPC channel；Electron main 会先校验 main-frame 发送方与内部权威，再调用 `ctx.connection.fetch()`。Renderer 侧 `ElectronApiClient` 把每个 Host URL 固定到 `http://dsh.internal`，经 preload 发送请求元数据，只接收一次响应元数据，再通过私有 `MessagePort` 逐个拉取 body chunk。事件 endpoint 直接使用 API Proxy Fetch codec，因此 Electron 载体不会打开 HTTP 或 WebSocket listener。临时全局 Fetch adapter 会把同文档请求与 `http://dsh.internal` 请求改写到该源；renderer 的其他请求继续使用 Chromium 原生 Fetch 实现。
