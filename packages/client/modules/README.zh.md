...
Node 侧会扫描已启用的 Loader 配置项以发现 web `dsh.client` 包，解析每个 `exports["./client"]`，并把构建后的 bundle 哈希写入启动图。Web server 通过 `/plugins` 提供这些文件与 sourcemap；Electron 主进程则经私有 `dsh://bundle` 协议消费同一组解析路径，不注册 route，也不开放 socket。源码启动会把宿主侧导入映射到 TypeScript 源码，但仍消费这一构建后的客户端导出；缺失文件共享一条构建说明，随后以包／路径列表列出各项，而无关的文件系统错误仍是独立故障。包元数据从 profile 配置树解析；该配置树声明已组装的插件，并作为隔离 pnpm 安装的锚点。
