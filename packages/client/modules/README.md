...
The Node half scans enabled Loader entries for web `dsh.client` packages, resolves each `exports["./client"]`, and hashes the built bundle into the boot graph. The Web server publishes those files and source maps under `/plugins`; the Electron main process consumes the same resolved paths through its private `dsh://bundle` protocol, without registering routes or opening a socket. Source launch maps host imports to TypeScript source but still consumes this built client export; missing files share one build instruction followed by a package/path list, while unrelated filesystem errors remain separate failures. Package metadata resolves from the profile config tree, which declares the composed plugins and anchors isolated pnpm installs.
