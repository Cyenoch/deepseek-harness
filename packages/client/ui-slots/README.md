...
| store | `PropsStoreFor<K, H>` | the declared handle and slot scope: `useStore` selector hook + draft-stripped `actions` |
...
The store family (`defineStore` spec in / `StoreHandle<T, A>` out) types the store seat: `init` infers the state schema, `actions` is the complete draft-transform write set, and `BakedActions` strips the draft parameter into the callbacks components and inject factories receive. `PropsStoreFor` selects the ordinary face for root/strict-session entries and the maybe-store face for `session-maybe`: its selector Hook is always callable across blank-to-first-session adoption and returns `undefined` while no session owns an instance, while actions remain absent. This preserves React Hook order without inventing a snapshot. The `defineStore` value implementation lives in the runtime package (the engine's home) and satisfies the `DefineStore` contract exported here. Engine products and the renderer host contract carry bare snapshot sources (`getSnapshot`/`subscribe`), never React hooks — hook binding belongs to the render machinery; only the props-contract Hook types live here.
