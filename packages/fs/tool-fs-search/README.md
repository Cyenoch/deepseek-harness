...
- **The packaged binary is fixed at dependency version** — `@vscode/ripgrep` covers the platforms it ships (macOS/Linux/Windows, x64/arm64); an unsupported platform or a corrupted install fails calls with `SEARCH_FAILED`. `DSH_RIPGREP_PATH`, when set, must name an absolute regular file and is used instead of the npm package; a missing, relative, or non-file value fails and does not fall back. Remote or virtual filesystems need a co-located workspace or another search consumer.
