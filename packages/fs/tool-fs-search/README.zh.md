...
- **打包二进制固定在依赖版本上**——`@vscode/ripgrep` 覆盖其随附的平台（macOS/Linux/Windows，x64/arm64）；不支持的平台或损坏的安装会以 `SEARCH_FAILED` 使调用失败。若设置了 `DSH_RIPGREP_PATH`，它必须是绝对路径上的普通文件，并取代 npm 包；缺失、相对或非文件值会失败，且不会回退。远程或虚拟文件系统需要共置的工作区或另一个搜索消费方。
