...
- **`stat` / `lstat`** — return target metadata or `undefined` when absent. `stat` reports `FsInfo` for an already resolved target (`version` = an opaque token derived from device, inode, size, modification time, and change time; `type` of `file`/`directory`/`other`; byte `size`); path-shaped `lstat` reports `FsPathInfo` without following the final symlink and can therefore return `symlink`. Native Node metadata uses nanosecond timestamps. A Node-compatible virtual filesystem that returns ordinary numeric `Stats` for the bigint overload, including Electron ASAR, uses millisecond timestamps instead. Both operations check cancellation before and after their asynchronous metadata probe, so an abort that lands in flight reports `FS_ABORTED` rather than stale absence.
...
- **Version tokens depend on filesystem metadata** — they combine device, inode, size, mtime, and ctime at the precision the backing filesystem exposes; a storage layer that cannot update any of those facts for a rewrite can still defeat the stale guard.
