...
export const CLIENT_SLOT_API: readonly ClientSlotEntry[] = [
  ...
  },
  {
    ...
    declaredBy: 'an entry in \'conversation.session.header\' (client-ui-conversation), so it exists while that entry is mounted',
    occupants: [
      'client-ui-sidepanel SidePanelToggle id \'sidepanel-toggle\'',
    ],
    replaceRisk: 'none',
  ...
  },
  {
    ...
    declaredBy: 'an entry in \'root\' (client-ui-layout), so it exists while that entry is mounted',
    occupants: [
      'client-ui-sidepanel SidePanelRoot',
    ],
    replaceRisk: 'shadows-shipped-ui',
  ...
  },
  {
    ...
    keyDomain: 'open: any string the owner dispatches (no compile-time key set), already taken: btw, session-log, terminal, trajectory',
    hookContext: '',
    slotInject: '',
    declaredBy: 'an entry in \'sidepanel\' (client-ui-sidepanel), so it exists while that entry is mounted',
    occupants: [
      'client-ui-sidepanel BtwApp key \'btw\'',
      'client-ui-sidepanel TerminalApp key \'terminal\'',
      'client-ui-trajectory TrajectoryView key \'trajectory\'',
      'session-log-export SessionLogSidepanelApp key \'session-log\'',
    ],
    replaceRisk: 'shadows-shipped-ui',
    example: 'return {\n  inject: [\'slots\'],\n  apply(ctx) {\n    ctx.slots.inject(\'sidepanel.app\', () => ctx.slots.register(\n      { name: \'sidepanel.app\', key: \'<one key the owner dispatches>\' },\n      () => React.createElement(\'div\', null, \'hello\'),\n    ))\n  },\n}',
    source: 'packages/client/ui-sidepanel/src/client/contract/slots.ts:32',
  },
  {
    key: 'sidepanel.launchpad',
    kind: 'list',
    scope: 'session-maybe',
    summary: 'Launchpad entries: the bookmark cards a user picks to open a tab.',
    doc: 'Launchpad entries: the bookmark cards a user picks to open a tab.\nEach entry knows its own identity and calls the owner `open` with the\ntab to open. Declared by ui-sidepanel\'s shell entry.',
    registerOptions: [
      {
        name: 'id',
        requirement: 'required',
    ...
    declaredBy: 'an entry in \'sidepanel\' (client-ui-sidepanel), so it exists while that entry is mounted',
    occupants: [
      'client-ui-sidepanel BtwLaunchCard id \'btw\'',
      'client-ui-sidepanel TerminalLaunchCard id \'terminal\'',
      'client-ui-trajectory TrajectoryLaunchCard id \'trajectory\'',
      'session-log-export SessionLogLaunchCard id \'session-log\'',
    ],
    replaceRisk: 'none',
    example: 'return {\n  inject: [\'slots\'],\n  apply(ctx) {\n    ctx.slots.inject(\'sidepanel.launchpad\', () => ctx.slots.register(\n      { name: \'sidepanel.launchpad\', id: \'my-entry\', order: 100, label: \'My entry\' },\n      () => React.createElement(\'div\', null, \'hello\'),\n    ))\n  },\n}',
    source: 'packages/client/ui-sidepanel/src/client/contract/slots.ts:38',
  },
  {
