...
export class LocalBashExecutor extends ShellExecutor {
  ...
  protected async runArgv(
    ...
    const handle = this.ctx.subprocess.spawn(this.spawnSpec(spec, argv, spec.stdoutMaxBytes, d.signal, environment))
  ...
  protected startArgv(
    ...
    const running = this.ctx.subprocess.spawn(this.spawnSpec(spec, argv, this.config.maxOutputBytes, spec.signal, environment))
