...
Persistent shell backend for `ctx.terminals` over `ctx.subprocess.spawnTerminal`. It starts model sessions in a controlled Bash and human attachments in the operating-system account's login shell under the shared `ctx.sandboxPolicy`, retains bounded line-oriented model output plus a bounded raw VT stream for human attachment, and detects readiness while the subprocess provider owns PTY allocation, environment scrubbing, foreground process groups, signalling, resizing, and complete terminal-session cleanup. The same PTY backend therefore composes with local or remote execution-world providers.

## Plugin (`terminal-bash`)

The plugin injects `terminals`, `sandboxPolicy`, and `subprocess`, then registers the configured backend type (`shell`). Model presentation uses `shellPath` and `shellArgs` (controlled Bash by default). Human presentation resolves the operating-system account shell and starts it as a login shell; `humanShellPath` and `humanShellArgs` override that choice. Human sessions keep their own prompt, profile, pager, and other shell behavior instead of receiving the model-only `PS1`, `PROMPT_COMMAND`, `PAGER`, and `GIT_PAGER` overrides. `danger-full-access` starts the shell directly without requiring a sandbox provider; confined modes require a same-world `ctx.sandbox` and wrap the exact shell argv through it, failing before spawn when none is mounted. At spawn, one `ctx.sandboxPolicy.resolve({ session })` call supplies both the effective mode and the session workspace root; the same root is the default shell cwd when the caller omits one. A change to a different effective mode is rejected before its `sandbox/mode` event commits while that owner has an open PTY or a spawn in progress; the fence is attached to the exact owner and therefore outlives a provider reload that retains existing sessions. Wait for creation to settle and close the sessions before changing modes, so a terminal opened with wider access cannot survive a downgrade.
...
Human presentation sets `TERM=xterm-256color` and exposes the decoded pre-sanitization VT stream. Each reader supplies its prior monotonic string cursor; a read waits cancellation-safely for output or final process state, reports a retention gap, and returns at most `maxReadBytes`. The shared retention cap remains `scrollbackMaxBytes`. Raw input has no newline conversion, resize delegates to the subprocess handle when available, and both reject once cleanup begins; late output callbacks after close are ignored.
...
- Model-facing output remains line-oriented and normalized; human attachment receives raw VT data and supports alternate-screen programs to the extent implemented by its renderer and subprocess provider.
