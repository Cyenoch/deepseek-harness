...
这是一个基于 `ctx.subprocess.spawnTerminal`、为 `ctx.terminals` 提供的持久 shell 后端。它在共享 `ctx.sandboxPolicy` 下为模型会话启动受控 Bash，并为人工附加启动操作系统账户的登录 shell；同时为模型保留有界的逐行输出，为人工附加保留有界的原始 VT 流，并检测就绪状态。进程管理提供方负责 PTY 分配、环境清理、前台进程组、信号发送、尺寸调整和完整终端会话清理。因此，同一个 PTY 后端可以与本地或远程执行世界提供方组合。

## 插件（`terminal-bash`）

该插件注入 `terminals`、`sandboxPolicy` 和 `subprocess`，然后注册所配置的后端类型（`shell`）。模型呈现使用 `shellPath` 和 `shellArgs`（默认是受控 Bash）。人工呈现会解析操作系统账户的 shell 并以登录 shell 启动；`humanShellPath` 和 `humanShellArgs` 可覆盖该选择。人工会话保留自己的提示符、profile、pager 和其他 shell 行为，不接收只供模型使用的 `PS1`、`PROMPT_COMMAND`、`PAGER` 与 `GIT_PAGER` 覆盖。`danger-full-access` 无需沙箱提供方即可直接启动 shell；受限模式要求同一执行世界中存在 `ctx.sandbox`，并通过它包装确切的 shell argv，未挂载时会在 spawn 前失败。spawn 时，一次 `ctx.sandboxPolicy.resolve({ session })` 调用会同时给出实际模式与会话工作区根目录；调用方省略 cwd 时，同一根目录也是 shell 的默认 cwd。当某个所有者存在开放的 PTY 或正在进行 spawn 时，如果配置变更会得到不同的实际模式，系统会在对应 `sandbox/mode` 事件提交前拒绝该变更。该限制绑定到确切所有者，因此即使提供方重新加载并保留现有会话，它仍然有效。更改模式前，请等待创建完成并关闭会话，避免以更宽权限打开的终端在权限降级后继续存在。
...
人工呈现设置 `TERM=xterm-256color`，并公开解码后、清理前的 VT 流。每个读取方提供上次的单调递增字符串游标；读取会以可安全取消的方式等待输出或最终进程状态，报告保留缺口，并且每次最多返回 `maxReadBytes`。共享保留上限仍为 `scrollbackMaxBytes`。原始输入不转换换行，尺寸调整在可用时委托给 subprocess 句柄；清理开始后两者都会拒绝。关闭后的延迟输出回调会被忽略。
...
- 面向模型的输出仍按行规范化；人工附加会接收原始 VT 数据，并在渲染器与 subprocess 提供方实现范围内支持备用屏幕程序。
