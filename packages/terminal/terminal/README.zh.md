...
- 后端可通过 `supportsInteractive` 与 `interactive` 传输选择支持人工附加。生成的 Agent 作用域 `terminals` Remote namespace 会列出适用后端、附加稳定的所有者本地名称、转发有界且按游标寻址的原始 VT 输出与原始输入、在支持时调整 PTY 尺寸，并通过同一注册表生命周期关闭它。
- 人工附加不是第二条权限路径：每个 Remote 方法都会解析确切的活跃 Agent，id 仍限定于所有者范围，输入具有请求大小上限；新建 PTY 的初次尺寸同步若失败，服务会回滚该 PTY。
- `kill()` 与 dispose 只会在后端捕获的进程树完全停稳后完成。清理失败会以拒绝结束，而非声称成功；同时它会清除匹配的后端和注册表限制，使后续关闭能够重试，且不会干扰较新的尝试。

该 seam 不包含 `node-pty`、沙箱、工具 schema、提示词、任务或终端渲染策略。实现负责终端机制；消费方负责模型呈现、人工 VT 渲染和可选的后台任务注册。浏览器安全的 Remote payload 位于 `./remote-types`，与只供 Host 使用的后端类型分离。
...
- 一个具名 PTY 无法同时服务模型 send 与人工输入流；模型 send 占用会话时，交互输入会失败。
