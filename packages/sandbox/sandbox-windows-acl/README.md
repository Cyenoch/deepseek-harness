...
Ordinary Node and source checkouts keep that `[node, runner.js]` prefix. Electron keeps the same runner and arguments: because its `process.execPath` names the application executable, the local provider adds `ELECTRON_RUN_AS_NODE=1` only to the runner child's environment so Electron executes the script as Node. A configured `DSH_WINDOWS_ACL_RUNNER` override must name an absolute regular file; a missing, relative, or non-file value fails and does not fall back.
