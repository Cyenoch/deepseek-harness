...
普通 Node 与源码检出保持该 `[node, runner.js]` 前缀。Electron 也保留同一个 runner 与参数：由于其 `process.execPath` 指向应用可执行文件，本地提供方只向该 runner 子进程的环境添加 `ELECTRON_RUN_AS_NODE=1`，使 Electron 按 Node 方式执行脚本。已配置的 `DSH_WINDOWS_ACL_RUNNER` 覆盖必须是绝对路径上的普通文件；缺失、相对或非文件值会失败，且不会回退。
