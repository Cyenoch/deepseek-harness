...
除非 `DSH_WINDOWS_ACL_RUNNER` 指向绝对路径上的普通文件，windows-acl 的调用前缀是 `[process.execPath, runner.js|runner.ts]`。在 Electron 中，`process.execPath` 是应用可执行文件，因此提供方只为该 runner 子进程附加 `ELECTRON_RUN_AS_NODE=1`；普通 Node 启动不携带额外环境。已配置但缺失、相对或非文件的覆盖会失败，而不会回退。
