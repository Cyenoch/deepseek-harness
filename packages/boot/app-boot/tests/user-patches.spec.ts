...
describe('boot with user patches', () => {
  ...
  it('watches add, failure, recovery, and removal through transactional HMR', { timeout: 20_000 }, async () => {
    ...
    // This case owns transactional patch replacement. The HMR config-path
    // suite owns native fs.watch behavior; polling keeps macOS watcher queue
    // pressure from obscuring the transaction assertions in the full suite.
    ...
    try {
      ...
      await dispose()
      const defaultFilename = join(tmp(), PROFILE_PATCH_FILENAME)
      writeFileSync(defaultFilename, '- id: noop\n  config:\n    value: identity\n')
      const disposeDefault = await watchUserPatches(ctx, { binName: NAME, filename: defaultFilename })
      try {
