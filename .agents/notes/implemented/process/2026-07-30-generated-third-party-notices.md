...
[`THIRD_PARTY_NOTICES.md`](../../../../THIRD_PARTY_NOTICES.md) is generated by [`scripts/gen-third-party-notices.ts`](../../../../scripts/gen-third-party-notices.ts) from the workspace manifests, `vendor/README.md`, the `pyproject.toml` files, and `pnpm-workspace.yaml`. Electron and its packaging dependencies enter through `apps/desktop/package.json` and the root npm lock like every other JavaScript dependency. The root README pair links the file from its License section; the unsigned desktop bundle ships the same generated file.
...
**Enumerate the full transitive closure.** The closure is thousands of packages, already recorded in the lock files with exact versions, and would bury the direct dependencies that a reader actually evaluates. The file points at `pnpm-lock.yaml`, `python/sdk/uv.lock`, and the `pnpm licenses list` renderer instead.
