...
We need a UI integration layer beyond the existing ACP/stdio baseline for the Web server, Electron, and later product clients. We call them Clients and require the following capabilities:
...
At the same time the physical channels differ per consumer (browser HTTP/WebSocket, Electron preload IPC, in-process fetch/SSE), so we also need a channel-independent message model and a single contract source of truth — "adding a method" and "swapping a carrier" must not entangle each other, and every message on the wire must be type-validatable, observable, and reconcilable.
...
- `apps/` holds the externally exported applications, assembled from Client / Host mixtures.
    ...
    - `apps/desktop` (`@deepseek-ai/dsh-desktop`) is the Electron application: its main process embeds the desktop profile Host and its preload exposes a typed IPC Fetch carrier to the shared Web client packages, with no HTTP server or backend child ([desktop host](2026-08-14-electron-embedded-desktop-host.md)).

```
apps/*  (applications: Web frontend, CLI launcher, Electron desktop application)
  │ consume
  ▼
...
| Carrier layer | `dsh-host-webserver` | Web HTTP and upgrade: static serving + `/api/*`→handler forwarding + WebSocket upgrade route + close semantics; plugin bundle endpoint + `__DSH_BOOT__` manifest injection (fed by the web plugin registry) | Browser access only; zero workspace dependencies (the registry arrives by structural injection); Electron uses the same Fetch dispatcher through preload IPC and does not mount this package |
...
| Application | `@deepseek-ai/dsh` (apps/cli) + `dsh-web-frontend` (apps/web, the vite application) + `@deepseek-ai/dsh-desktop` (apps/desktop) | Coarse bin dispatch + one assembly module per application (web.ts / headless.ts); the Vite app is a thin main over the `dsh-client-web` shell surface; Electron main embeds the desktop profile and owns native lifecycle plus preload authority | Applications use dynamic imports so they never load each other; workspace knowledge such as dist location stays in the app |
...
1. **Pick a Fetch carrier**: browser same-origin HTTP / Electron preload IPC / in-process `host.handler.fetch` injection / another transport-aspect subclass (see the "Subclass table" below).
2. **Write an assembly module under `apps/`**: `runProfile()` or `startHost()` + a client subclass + the application's private signal/print/exit semantics; a mixture never becomes a package — assembly is written in the app.
3. **Import `dsh-host-webserver` only if you need HTTP carriage**, otherwise zero ports.

The three applications preserve the division: Web mounts the Host, Web carrier, and browser composition; Electron embeds the Host and carries Fetch over preload IPC with zero ports; `dsh --profile headless` mounts a direct core runner with zero Host, HTTP, or browser layer. ACP-class protocol bridges do not follow the client-carrier checklist: they expose core to the external ecosystem and mount directly via `ctx.plugin(entry-point plugin)` without Fetch.
