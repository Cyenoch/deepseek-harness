...
**Make `rgPath` a tool config field or use the deployment override to substitute a stand-in in tests and snapshots.** Rejected: model-facing configuration does not own executable placement, and tests can pin the real binary directly through fixture mtimes. `DSH_RIPGREP_PATH` remains a bootstrap-only deployment escape hatch for runtimes such as packaged Electron, not a test substitution mechanism.
...
- The desktop artifact smoke boots the packaged profile and executes `glob` and `grep` through a real Standard Agent, so ASAR path resolution, physical binary placement, subprocess launch, and tool decoding are one release-gating path.
